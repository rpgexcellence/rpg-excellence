      ...
      <div style={{ borderTop: "1px solid #e2e9f1", padding: "20px" }}>
        ...
        <form action={saveSoaControl}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
            ...
            <label style={labelStyle}>Residual risk
              <textarea name={`residual_risk_${key}`} defaultValue={row.residual_risk ?? ""} rows={2} style={field} disabled={!canEdit} />
            </label>
            ...
            <label style={labelStyle}>Action target date
              <input type="date" name={`target_date_${key}`} defaultValue={row.target_date ?? ""} style={field} disabled={!canEdit} />
            </label>
