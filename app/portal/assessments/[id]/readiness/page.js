...
export default async function CertificationReadinessPage({
  ...
  /*
   * related_finding_id is the canonical
   * Management Action Plan relationship.
   *
   * finding_id remains as a compatibility
  ...
  /*
   * A management action is treated as
   * completed when it has reached either
  ...
  /*
   * Operational gates and readiness-strength
   * gates are deliberately shown separately.
   *
   * This means a user can see that all formal
   * NC/action blockers are controlled while
   * the underlying maturity/readiness scores
  ...
  const checks = [
    ...
    /*
     * This check explains why all operational
     * gates can pass while the overall RPG
     * recommendation still requires further
     * improvement.
     *
     * Minimum progression strength:
     *
     * Assessment score: 60%
     * Management readiness: 65%
     */
    {
      label:
        "Readiness strength sufficient",
      ...
      detail:
        ...
        managementReadinessScore !==
          null
          ? `${overallScore}% assessment score / ${managementReadinessScore}% management readiness`
          : "Readiness strength not yet available",
