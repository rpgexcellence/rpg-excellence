...
const catalogue = [
  {
    code: "RA-INITIAL-001",
    product: "risk-assessment-initial",
  ...
  },
  {
    ...
    product: "risk-assessment-refresher",
...
function CourseCard({ item, course, enrolment }) {
  ...
  const href = enrolment ? "/portal/health-safety/training/" + enrolment.id : null;
  const displayedPrice = course
    ? `£${(course.price_pence / 100).toFixed(2)} + VAT`
    : item.price;
  return <article className="taCourse">
    ...
    {enrolment ? <><div className="taProgress"><i style={{ width: progress + "%" }} /></div><div className="taProgressText"><span>{progress}% complete</span><span>{enrolment.completed_at ? "Completed " + formatDate(enrolment.completed_at) : "Continue learning"}</span></div></> : <div className="taPrice">{displayedPrice}</div>}
    {enrolment ? <Link className="taButton" href={href}>{progress ? "Continue course →" : "Start course →"}</Link> : course ? <TrainingPurchaseButton className="taButton" course={item.product}>Buy course →</TrainingPurchaseButton> : <Link className="taButton" href="/en/hs-hub/training">Course details →</Link>}
  </article>;
}
