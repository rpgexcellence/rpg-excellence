...
function validateForApproval(assessment, hazards, actions, requireReviewControls = true) {
  const failures = [];
  if (!assessment.task_description || !assessment.assessor_name || !assessment.assessment_date) failures.push("Assessment context, assessor and assessment date must be complete.");
  if (requireReviewControls && !assessment.review_date) failures.push("A planned review date must be set at the approval gate.");
...
async function submitForReview(formData) {
  ...
  const failures = validateForApproval(record.assessment, record.hazards, record.actions, false);
...
async function recordReviewDecision(formData) {
  ...
  const reviewFrequency = String(formData.get("review_frequency_months") || "").trim();
  const record = await getControlledRecord(supabase, user.id, id);
  if (!record.assessment) throw new Error("Risk assessment could not be accessed.");
  ...
  if (decision === "approved") {
    if (!reviewDate) throw new Error("Set the planned review date before approving the assessment.");
    if (!reviewFrequency) throw new Error("Select a review frequency or event-based review.");
    const failures = validateForApproval({ ...record.assessment, review_date: reviewDate }, record.hazards, record.actions, true);
    if (failures.length) throw new Error(failures.join(" "));
  }
  const update = decision === "approved"
    ? { status: "approved", approver_name: approverName, approval_comment: comment || null, review_date: reviewDate, review_frequency_months: reviewFrequency === "event_based" ? null : Number(reviewFrequency), approved_by: user.id, approved_at: new Date().toISOString() }
...
export default async function ReviewApprovalPage({ params }) {
  ...
  const failures = validateForApproval(assessment, hazards, actions, !["draft", "changes_required", "in_review"].includes(assessment.status));
  const elevated = hazards.filter((hazard) => (hazard.residual_score || 0) >= 10);
  ...
  `}</style><div className="rrShell">
    ...
    {assessment.status === "in_review" && <section className="rrPanel"><div className="rrHead"><h2>Record review decision</h2><p>The approver confirms suitability and sets the controlled review arrangements based on the residual-risk position and expected change.</p></div><form className="rrForm" action={recordReviewDecision}><input type="hidden" name="assessment_id" value={id}/><div className="rrField"><label htmlFor="approver_name">Competent approver name *</label><input id="approver_name" name="approver_name" required/></div><div className="rrField"><label htmlFor="review_date">Planned review date *</label><input id="review_date" name="review_date" type="date" defaultValue={assessment.review_date || ""} required/></div><div className="rrField"><label htmlFor="review_frequency_months">Review frequency *</label><select id="review_frequency_months" name="review_frequency_months" defaultValue={assessment.review_frequency_months ? String(assessment.review_frequency_months) : ""} required><option value="" disabled>Select review arrangement</option><option value="event_based">Event-based / following significant change</option><option value="3">Every 3 months</option><option value="6">Every 6 months</option><option value="12">Every 12 months</option><option value="24">Every 24 months</option><option value="36">Every 36 months</option></select></div><div className="rrField full"><label htmlFor="approval_comment">Approval comment / changes required</label><textarea id="approval_comment" name="approval_comment"/></div><div className="rrDecisions"><button className="rrButton" name="decision" value="approved">Approve controlled assessment</button><button className="rrButton red" name="decision" value="changes_required">Return for changes</button></div></form></section>}
    ...
    {["approved", "communicated"].includes(assessment.status) && <section className="rrPanel"><div className="rrHead"><h2>Controlled approval record</h2></div><div className="rrHistory"><div><strong>Approved by</strong><span>{assessment.approver_name || "—"}</span></div><div><strong>Approved</strong><span>{formatDate(assessment.approved_at)}</span></div><div><strong>Review date</strong><span>{formatDate(assessment.review_date)}</span></div><div><strong>Review frequency</strong><span>{assessment.review_frequency_months ? `Every ${assessment.review_frequency_months} months` : "Event-based / following significant change"}</span></div><div><strong>Approval comment</strong><span>{assessment.approval_comment || "No additional comment"}</span></div>{assessment.communicated_at && <div><strong>Communicated</strong><span>{formatDate(assessment.communicated_at)}</span></div>}</div></section>}
  </div></main>;
}
