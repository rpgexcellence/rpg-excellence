...
export default async function RiskActionVerificationPage({ params }) {
  ...
  `}</style><div className="havShell">
    ...
    <section className="havPanel"><div className="havHead"><h2>Control requirement</h2><p>The action must be implemented as described or formally revised through the assessment owner.</p></div><div className="havDetails"><div><span>PRIORITY</span><strong>{label(action.priority)}</strong></div><div><span>RESPONSIBLE EMAIL</span><strong>{action.responsible_email || "—"}</strong></div><div><span>INTERIM CONTROLS</span><strong>{action.interim_controls || "—"}</strong></div><div><span>ASSESSMENT</span><strong><Link href={"/portal/health-safety/risk-assessment/" + assessment.id}>{assessment.assessment_reference}</Link></strong></div></div>{hazard && <div className="havRisk" style={{marginTop:14}}><div><strong>Residual-risk position</strong><small>{hazard.residual_likelihood} likelihood × {hazard.residual_severity} severity</small></div><b>{hazard.residual_score || "—"}</b></div>}</section>
