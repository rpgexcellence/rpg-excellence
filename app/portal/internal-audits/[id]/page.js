        ...
        <div className="workspaceGrid">
          ...
          <section className="mainPanel">
            ...
            {gate === "report" ? <>
              ...
              <section className="fieldworkModule"><div className="fieldworkHead"><div><span className="panelKicker">01 · Report content</span><h3>Controlled audit report</h3><p>Record the report narrative, limitations, distribution and overall management-system conclusion.</p></div><span className="countBadge">{reportControl?.status?.replaceAll("_", " ") || "not started"}</span></div>
...
function D6AuditorVerificationPanel({ auditId, finding, access, actions, evidence, evidenceUrls }) {
  const pending = actions.filter((action) => action.d6_submitted_at && !["effective", "effective_verified"].includes(action.effectiveness_result)).length;
  return <section className="fieldworkModule" style={{ marginTop: 18 }}>
    ...
    <div className="findingList">{actions.map((action) => {
      const actionEvidence = evidence.filter((item) => item.action_id === action.id);
      return <article className="findingCard" key={action.id} style={{ display: "block" }}>
      ...
      <div className={actionEvidence.length ? "coming" : "notice error"} style={{ marginTop: 12 }}>
        ...
        {actionEvidence.length ? <div style={{ display: "grid", gap: 8, marginTop: 8 }}>{actionEvidence.map((item) => <div key={item.id}><a href={evidenceUrls.get(item.storage_path) || "#"} target="_blank" rel="noreferrer">{item.reference || "Evidence record"}</a>{item.description ? <span> — {item.description}</span> : null}{item.evidence_date ? <span> · {displayDate(item.evidence_date)}</span> : null}{item.strength ? <span> · {item.strength} strength</span> : null}</div>)}</div> : <div style={{ marginTop: 6 }}>No evidence is linked to this action. Select “Unable to verify—insufficient evidence” unless the action owner supplies an adequate record.</div>}
      </div>
      {action.d6_submitted_at ? <form action={verifyD6CorrectiveAction} style={{ marginTop: 16 }}>
        ...
        <div className="grid3"><label className="field"><span>Auditor effectiveness decision *</span><select name="effectiveness_result" required defaultValue={["effective", "effective_verified"].includes(action.effectiveness_result) ? "effective_verified" : ""}><option value="" disabled>Select decision</option><option value="effective_verified">Effective—verified</option><option value="partially_effective">Partially effective—further action required</option><option value="not_effective">Not effective—reopen corrective action</option><option value="unable_to_verify">Unable to verify—insufficient evidence</option></select></label><label className="field"><span>Residual risk *</span><select name="residual_risk" required defaultValue={action.residual_risk || ""}><option value="" disabled>Select risk</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></label><label className="field"><span>Verification method *</span><textarea name="effectiveness_verification_method" required defaultValue={action.effectiveness_verification_method || ""} placeholder="Record sample, observation, test, interview or performance-trend review" /></label></div>
