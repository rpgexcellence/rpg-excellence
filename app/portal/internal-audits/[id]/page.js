        ...
        <div className="workspaceGrid">
          ...
          <section className="mainPanel">
            ...
            {gate === "report" ? <>
              ...
              {query?.saved === "ai_draft" ? <div className="notice" role="status"><strong>AI-assisted narrative drafted.</strong> Review and edit every section below. The lead auditor remains accountable for accuracy and approval.</div> : null}
              {query?.report_error === "ai_not_configured" ? <div className="notice error" role="alert"><strong>AI drafting is not configured.</strong> Add the OPENAI_API_KEY environment variable in Vercel, then redeploy.</div> : null}
              {query?.report_error === "ai_insufficient_evidence" ? <div className="notice error" role="alert"><strong>More audit results are required.</strong> Record at least one assessed criterion before generating a report draft.</div> : null}
              {query?.report_error === "ai_failed" ? <div className="notice error" role="alert"><strong>The AI draft could not be generated.</strong> The controlled audit record was not changed. Try again or prepare the narrative manually.</div> : null}
              <section className="fieldworkModule" style={{marginBottom:22}}><div className="fieldworkHead"><div><span className="panelKicker">01 · AI-assisted drafting</span><h3>Build narrative from the controlled audit record</h3><p>Generate a concise first draft from approved scope, assessed criteria, auditor notes, findings and evidence metadata. AI cannot add or alter controlled findings, classifications or objective evidence.</p></div><span className="countBadge">Human review required</span></div>
                <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.5fr) minmax(280px,.5fr)",gap:18,padding:22,alignItems:"stretch"}}><div className="coming" style={{margin:0}}><strong>Drafts four editable sections</strong><br />Executive summary, methodology and sampling, limitations and exclusions, and the overall management-system conclusion. Existing text is preserved unless you explicitly allow replacement.</div><form action={generateAuditReportAiDraft} style={{display:"flex",flexDirection:"column",justifyContent:"space-between",gap:14}}><input type="hidden" name="audit_id" value={id} /><label className="check" style={{margin:0}}><input type="checkbox" name="replace_existing" /><span><strong>Replace existing narrative</strong><br />Leave clear to fill blank sections only.</span></label><button className="button primary">Generate AI Report Draft</button></form></div>
              </section>
