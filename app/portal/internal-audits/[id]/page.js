...
export default async function InternalAuditWorkspace({ params, searchParams }) {
  ...
  const eligibleAuditors = (eligibleAuditorsResult.data ?? []).filter((auditor) => {
    const authorised = new Set((auditor.standard_authorisations ?? []).filter((item) => item.authorisation_status === "authorised" && (!item.authorised_until || item.authorised_until >= today)).map((item) => item.standard_id));
    return standardIds.every((standardId) => authorised.has(standardId));
  });
          ...
          <section className="mainPanel">
            ...
            {gate === "team" ? <>
              ...
              <form action={addAuditTeamMember}>
                <input type="hidden" name="audit_id" value={id} /><div className="grid3"><label className="field"><span>Verified auditor *</span><select name="auditor_register_id" required defaultValue=""><option value="">Select verified auditor</option>{eligibleAuditors.map((auditor) => <option key={auditor.id} value={auditor.id}>{auditor.full_name} · valid to {auditor.verified_until}</option>)}</select><small>{eligibleAuditors.length} auditor{eligibleAuditors.length === 1 ? "" : "s"} verified for all selected standards.</small></label><label className="field"><span>Audit role *</span><select name="audit_role" defaultValue="auditor">{Object.entries(ROLE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="field"><span>Assigned audit scope</span><textarea name="assigned_scope" required placeholder="Processes, clauses, sites or activities assigned to this auditor" /></label><label className="field"><span>Standards competence</span><textarea name="standards_competence" placeholder="Optional audit-specific competence rationale; register verification is inherited." /></label><label className="field"><span>Sector competence</span><textarea name="sector_competence" /></label><label className="field"><span>Technical competence</span><textarea name="technical_competence" /></label><label className="check"><input type="checkbox" checked readOnly /><span>Current register verification and standard authorisation confirmed by the system</span></label><label className="check"><input type="checkbox" name="independence_confirmed" required /><span>Independence and impartiality confirmed for this audit</span></label><label className="check"><input type="checkbox" name="confidentiality_confirmed" required /><span>Confidentiality obligations confirmed</span></label></div>{eligibleAuditors.length ? <div className="actionBar"><button className="button primary">Assign Verified Auditor</button></div> : <div className="notice error"><strong>No eligible auditor is available.</strong> Complete or renew verification and authorise the required standards in <Link href="/portal/internal-auditor-verification">Internal Auditor Verification</Link>.</div>}
              </form>
