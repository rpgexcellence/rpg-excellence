...
export default async function InternalAuditWorkspace({ params, searchParams }) {
  ...
  const eligibleAuditors = eligibleAuditorsResult.data ?? [];
  ...
  return (
    <main className="workspacePage">
      ...
      <div className="workspaceShell">
        ...
        {query?.team_warning === "standards_coverage" ? <div className="notice error" role="alert"><strong>Team standards coverage is incomplete.</strong> The combined verified team must cover every standard selected for this audit. Assign another suitably authorised auditor or extend the controlled competence scope.</div> : null}
        ...
        <div className="workspaceGrid">
          ...
          <section className="mainPanel">
            ...
            {gate === "team" ? <>
              <div className="panelKicker">Gate 02 · Team assignment</div><h2>Build a competent and independent audit team</h2><p className="panelLead">Only active auditors with current verification and authorisation for every selected standard are available. Independence and confidentiality remain audit-specific decisions.</p>
              ...
              <form action={addAuditTeamMember}>
                <input type="hidden" name="audit_id" value={id} /><div className="grid3"><label className="field"><span>Verified auditor *</span><select name="auditor_register_id" required defaultValue=""><option value="">Select verified auditor</option>{eligibleAuditors.map((auditor) => <option key={auditor.id} value={auditor.id}>{auditor.full_name} · valid to {auditor.verified_until}</option>)}</select><small>{eligibleAuditors.length} current verified auditor{eligibleAuditors.length === 1 ? "" : "s"}. Assign only standards within the selected person’s authorised scope.</small></label><label className="field"><span>Audit role *</span><select name="audit_role" defaultValue="auditor">{Object.entries(ROLE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="field"><span>Assigned audit scope</span><textarea name="assigned_scope" required placeholder="Processes, clauses, sites or activities assigned to this auditor" /></label></div><fieldset className="section"><legend><strong>Standards assigned to this auditor *</strong></legend><div className="grid2">{(audit.internal_audit_selected_standards ?? []).map((row) => <label className="check" key={row.standard_id}><input type="checkbox" name="assigned_standard_ids" value={row.standard_id}/><span>{row.internal_audit_standard_catalogue?.display_name || row.internal_audit_standard_catalogue?.standard_code || "Selected audit standard"}</span></label>)}</div></fieldset><div className="grid3"><label className="field"><span>Standards competence rationale</span><textarea name="standards_competence" placeholder="Optional audit-specific rationale; controlled authorisation is checked by the system." /></label><label className="field"><span>Sector competence</span><textarea name="sector_competence" /></label><label className="field"><span>Technical competence</span><textarea name="technical_competence" /></label><label className="check"><input type="checkbox" checked readOnly /><span>Current register verification confirmed by the system</span></label><label className="check"><input type="checkbox" name="independence_confirmed" required /><span>Independence and impartiality confirmed for this audit</span></label><label className="check"><input type="checkbox" name="confidentiality_confirmed" required /><span>Confidentiality obligations confirmed</span></label></div>{eligibleAuditors.length ? <div className="actionBar"><button className="button primary">Assign Verified Auditor</button></div> : <div className="notice error"><strong>No current verified auditor is available.</strong> Complete or renew verification in <Link href="/portal/internal-auditor-verification">Internal Auditor Verification</Link>.</div>}
              </form>
