    ...
    <main className="workspacePage">
      <style>{`
        ...
        *{box-sizing:border-box}.workspacePage{min-height:100vh;padding:25px clamp(16px,4vw,64px) 80px;background:linear-gradient(180deg,#eef4fb,#f8fafc 520px);color:var(--navy)}.workspaceShell{max-width:1720px;margin:auto}
        ...
        @media(max-width:1050px){.auditHeader,.workspaceGrid{grid-template-columns:1fr}.sideRail{display:none}.gateNav{grid-template-columns:repeat(5,200px);overflow-x:auto}.teamCard{grid-template-columns:1fr 1fr}.grid3{grid-template-columns:repeat(2,1fr)}}@media(max-width:700px){.workspacePage{padding:18px 12px 60px}.workspaceLogo{width:190px;height:auto}.auditHeader,.mainPanel{padding:24px 20px}.auditStandard,.grid2,.grid3,.planPreview,.teamCard{grid-template-columns:1fr}.actionBar{align-items:stretch;flex-direction:column}.button{width:100%}}
      `}</style>
