          ...
          <section className="mainPanel">
            ...
            {gate === "actions" ? <>
              ...
              <section className="fieldworkModule"><div className="fieldworkHead"><div><span className="panelKicker">01 · Assigned nonconformities</span><h3>Action-owner register</h3><p>Assign access, review submitted responses and continue detailed root-cause work in the linked CAPA–8D case.</p></div><span className="countBadge">{nonconformities.length} NCs</span></div>
                {nonconformities.length ? <div className="findingList">{nonconformities.map((finding) => { const access = actionAccessByFinding.get(finding.id); const ownerEvidence = access ? actionEvidenceByAccess.get(access.id) ?? [] : []; const rcaCase = rcaCasesById.get(finding.linked_rca_case_id); return <article id={`action-${finding.id}`} className={`findingCard ${finding.finding_type}`} key={finding.id} style={{display:"block"}}><div><b>{finding.finding_reference}</b><h4>{finding.title}</h4><p>{finding.failure_statement || finding.objective_evidence}</p></div>
                  ...
                  {access?.status === "submitted" ? <form action={reviewAuditActionResponse} style={{marginTop:16}}><input type="hidden" name="audit_id" value={id} /><input type="hidden" name="action_access_id" value={access.id} /><div className="grid3"><label className="field"><span>Auditor decision</span><select name="decision"><option value="accepted">Accept response</option><option value="returned">Return for revision</option></select></label><label className="field span2"><span>Auditor response *</span><textarea name="auditor_response" required /></label></div><div className="compactAction"><button className="button approve">Record Auditor Decision</button></div></form> : null}
                  {rcaCase ? <div style={{marginTop:14}}><Link className="button primary" href={`/portal/rca/${rcaCase.id}`}>Open Controlled CAPA–8D →</Link></div> : null}
