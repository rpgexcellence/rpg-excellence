          ...
          <section className="mainPanel">
            ...
            {gate === "actions" ? <>
              ...
              <section className="fieldworkModule"><div className="fieldworkHead"><div><span className="panelKicker">01 · Assigned nonconformities</span><h3>Action-owner register</h3><p>Assign access, review submitted responses and continue detailed root-cause work in the linked CAPA–8D case.</p></div><span className="countBadge">{nonconformities.length} NCs</span></div>
                {nonconformities.length ? <div className="findingList">{nonconformities.map((finding) => { const access = actionAccessByFinding.get(finding.id); const ownerEvidence = access?.status === "submitted" ? actionEvidenceByAccess.get(access.id) ?? [] : []; const rcaCase = rcaCasesById.get(finding.linked_rca_case_id); const d6Actions = (rcaActionsByCase.get(finding.linked_rca_case_id) ?? []).filter((action) => action.discipline === 5 && action.selection_status === "selected"); const d6Ready = d6Actions.length > 0 && d6Actions.every((action) => action.effectiveness_result === "effective_verified" && action.status === "verified" && action.verified_by && action.verified_at); const d6EvidenceForCase = d6EvidenceByCase.get(finding.linked_rca_case_id) ?? []; return <article id={`action-${finding.id}`} className={`findingCard ${finding.finding_type}`} key={finding.id} style={{display:"block"}}><div><b>{finding.finding_reference}</b><h4>{finding.title}</h4><p>{finding.failure_statement || finding.objective_evidence}</p></div>
