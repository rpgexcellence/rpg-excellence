...
export default async function InternalAuditCommandCentre({ searchParams }) {
  ...
  return (
    <main className="auditPage">
      ...
      `}</style>
      <style>{`@media(max-width:900px){.iaShell{display:block}.iaSidebar{position:static;width:100%;height:auto;padding:16px 20px}.iaBrand{margin:0;text-align:left;font-size:21px}.iaBrand:first-letter{font-size:inherit}.iaSideNav,.iaSideCaption,.iaSideStandard{display:none}.iaWorkspace{padding:20px 14px 60px}.iaTop>div:last-child{display:none}}@media(max-width:560px){#rpgIaWelcomeTitle{font-size:34px!important}.iaPanelHead,.iaForm{padding:21px 17px}.iaSectionTitle{align-items:flex-start;font-size:18px}.iaField input,.iaField select,.iaField textarea{font-size:16px}.iaStandard{padding:14px}.processGuidance{padding:14px}.processTools{grid-template-columns:1fr}.processSearch{grid-column:auto}.processToolButton{min-height:44px}.iaActionBar{position:static}}`}</style>
      ...
      <div className="iaShell">
        <aside className="iaSidebar">
          ...
          <nav className="iaSideNav" aria-label="Customer portal navigation">
            ...
            <div className="iaAuditSub"><Link href="/portal/internal-audits">Audit Command Centre</Link><Link href="/portal/internal-audit-programme">3-Year Audit Programme</Link><Link href="/portal/internal-audit-fmea-planning">FMEA Risk Planning</Link><Link href="/portal/internal-auditor-verification">Auditor Verification</Link></div>
