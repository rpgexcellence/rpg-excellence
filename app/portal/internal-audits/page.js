      ...
      <div className="iaShell">
        ...
        <section className="iaPanel">
          ...
          {audits.length === 0 ? <div className="iaNoAudits">No internal audits have been created yet. Your first governed audit will appear here.</div> : audits.map((audit) => {
            ...
            return <article className="iaAudit" key={audit.id}><div><strong className="iaAuditTitle">{audit.audit_reference} · {audit.title}</strong><span className="iaMuted">{TYPE_LABELS[audit.audit_type] ?? audit.audit_type} · {audit.audit_method}</span></div><div className="iaCriteria">{criteria.join(" · ") || "Criteria awaiting confirmation"}</div><div><span className="iaStatus">{STATUS_LABELS[audit.status] ?? audit.status}</span><div className="iaMuted" style={{marginTop:"6px"}}>Gate: {audit.current_gate}</div></div><div><strong>{formatDate(audit.planned_start_at)}</strong><div className="iaMuted">Planned start</div></div></article>;
          })}
        </section>
      </div>
    </main>
