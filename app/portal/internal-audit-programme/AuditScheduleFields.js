...
export default function AuditScheduleFields({ risks, cycleStart, cycleEnd, leadAuditor }) {
  ...
  return <>
    <div className="iapGrid3">
      ...
      <label className="iapField"><span>Planned start *</span><input name="planned_start" type="date" required min={cycleStart} max={cycleEnd} value={plannedStart} onChange={(event) => { setPlannedStart(event.target.value); if (!plannedEnd) setPlannedEnd(event.target.value); }} /></label>
      <label className="iapField"><span>Planned end *</span><input name="planned_end" type="date" required min={cycleStart} max={cycleEnd} value={plannedEnd} onChange={(event) => setPlannedEnd(event.target.value)} /></label>
