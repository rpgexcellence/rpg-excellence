...
export const metadata = {
  title: "Internal Auditor Refresher Certificate | RPG Excellence",
};
...
export default async function TrainingCertificatePage({ params }) {
  ...
  if (!user) {
    redirect(`/portal/login?next=/portal/internal-audit/training/${id}/certificate`);
  }
  ...
  if (enrolment.status !== "passed") {
    redirect(`/portal/internal-audit/training/${id}/assessment`);
  }
  ...
  return (
    <main className="tcPage">
      ...
      <div className="tcWrap">
        <nav className="tcNav">
          <Link href={`/portal/internal-audit/training/${id}/assessment`}>← Assessment record</Link>
          <a href={`/portal/internal-audit/training/${id}/certificate/pdf`}>Download PDF ↓</a>
        </nav>
        ...
        <article className="tcPaper">
          ...
          <p className="tcStatement">
            The learner completed the Internal Auditor Refresher programme and achieved the required standard in the final knowledge assessment.
          </p>
          ...
          <footer className="tcFoot">
            <span>Course version {certificate.course_version}<br/>RPG Excellence Internal Audit Academy</span>
            <span>Internal auditor refresher record<br/>Auditor competence must also be supported by appropriate experience, observed practice and ongoing evaluation.</span>
          </footer>
        </article>
      </div>
    </main>
