...
export const metadata = {
  title: "Internal Auditor Refresher Assessment | RPG Excellence",
};
...
async function submitAssessment(formData) {
  ...
  if (ownedEnrolment.status === "passed") {
    redirect(`/portal/internal-audit/training/${enrolmentId}/assessment?result=passed`);
  }
  ...
  const [courseResult, questionsResult, progressResult, attemptResult] = await Promise.all([
    admin
      ...
      .select("id,title,version,pass_mark,validity_months,course_code,academy_code")
  ...
  if (courseResult.data.course_code !== "IA-REFRESHER-001" || courseResult.data.academy_code !== "internal_audit") {
    throw new Error("This enrolment is not an Internal Auditor Refresher course.");
  }
  ...
  if (passed) {
    const certificateNumber = `RPG-IA-${now.slice(0, 4)}-${crypto.randomUUID().replaceAll("-", "").slice(0, 10).toUpperCase()}`;
  ...
  revalidatePath(`/portal/internal-audit/training/${enrolmentId}`);
  revalidatePath(`/portal/internal-audit/training/${enrolmentId}/assessment`);
  revalidatePath("/portal/internal-audit/training");
  redirect(`/portal/internal-audit/training/${enrolmentId}/assessment?result=${passed ? "passed" : "failed"}&score=${score}`);
}
...
export default async function TrainingAssessmentPage({ params, searchParams }) {
  ...
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/portal/login?next=/portal/internal-audit/training/${id}/assessment`);
  ...
  if (!["assessment_due", "failed", "passed"].includes(enrolment.status)) {
    redirect(`/portal/internal-audit/training/${id}`);
  }

  const [courseResult, questionsResult, attemptsResult, certificateResult] = await Promise.all([
    supabase
      .from("hs_training_courses")
      .select("id,title,description,pass_mark,version,course_code,academy_code")
  ...
  if (course.course_code !== "IA-REFRESHER-001" || course.academy_code !== "internal_audit") notFound();
  const questions = questionsResult.data || [];
    ...
    <main className="taPage">
      ...
      <div className="taWrap">
        <header className="taTop">
          <div>
            <small>RPG INTERNAL AUDIT ACADEMY · FINAL ASSESSMENT</small>
            <h1>{course.title}</h1>
            <p>Answer every question. A score of at least {course.pass_mark}% is required to pass.</p>
          </div>
          <Link className="taLink" href={`/portal/internal-audit/training/${id}`}>← Course content</Link>
        </header>
          ...
          <section className={`taResult ${passed ? "" : "fail"}`}>
            ...
            {certificate && (
              <div className="taCertificate">
                ...
                <Link href={`/portal/internal-audit/training/${id}/certificate`}>
