...
export default async function BillingSuccessPage({ searchParams }) {
  ...
  return <main style={{minHeight:"100vh",background:"#f4f7fb",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"Arial, sans-serif",padding:40}}><div style={{background:"#fff",maxWidth:700,width:"100%",borderRadius:18,padding:50,textAlign:"center",boxShadow:"0 15px 40px rgba(0,0,0,.08)"}}>
    ...
    {isAssessment && <div style={{background:"#eef6ff",border:"1px solid #cbdff8",borderRadius:12,padding:20,marginBottom:30,color:"#173c67",lineHeight:1.7}}><strong>{standard}</strong><br/>You have 30 days to complete the assessment and 90 days to manage findings, linked CAPA or 8D actions and effectiveness verification. Your assessment and report remain available afterward as read-only records.</div>}
