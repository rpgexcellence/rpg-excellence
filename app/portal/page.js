...
export default async function PortalPage({ searchParams }) {
  ...
  const workspaceTiles = [
    ["Internal Audit", "/portal/internal-audits", `${activeAudits.length} active · ${audits.length} total`, "AUD"],
    ["CAPA-8D", "/portal/rca", `${controlledActions.length} controlled actions · ${awaitingVerification.length} verification`, "8D"],
    ["Health & Safety", "/portal/health-safety", "Risk assessments, POWRA, permits and change", "H&S"],
    ["Information Security", "/portal/information-security", `${soaWorkspaces.length} SoA workspaces · risk and treatment`, "IS"],
    ["Business Continuity", "/portal/business-continuity", "Profiles, context, roles, learning and recovery", "BC"],
    ["Documents & Evidence", "/portal/documents", "Controlled documents and objective evidence", "DOC"],
    ["Training & Competence", "/portal/health-safety/training", "Learning, assessments and certificates", "L&D"],
    ["Gap Assessments", "/portal/history", `${assessments.length} assessments · ${standardsInUse.length} standards`, "GAP"],
  ];
      ...
      <div className="pdShell">
        ...
        <div className="pdMain">
          ...
          <section className="pdWorkspaceSection">
            ...
            <div className="pdWorkspaceGrid">{workspaceTiles.map(([title,href,detail,code])=><Link href={href} key={title}><b>{code}</b><div><strong>{title}</strong><p>{detail}</p><span>Open workspace →</span></div></Link>)}</div>
          </section>
