  ...
  if (!organization)
    return (
      <main className="pdOnboarding">
        <form action={createOrganization} className="pdOnboardingCard">
          <span>RPG EXCELLENCE</span>
          <h1>Create your organisation</h1>
          <p>
            Set up the controlled workspace before launching assessments and
            audits.
          </p>
          <input name="name" required placeholder="Organisation name" />
      ...
      <div className="pdShell">
        ...
        <div className="pdMain">
          <header className="pdTop">
            <div>
              <small>BUSINESS ASSURANCE COMMAND CENTRE</small>
            ...
            <div className="pdTopActions">
              ...
              <Link href="/portal/internal-audits" className="pdButton">
                Open audits →
              </Link>
            </div>
          </header>
          <section className="pdScoreBand">
            <div className="pdScoreCard">
              <div>
                <span>BUSINESS ASSURANCE SCORE</span>
                <h2>{maturity(assuranceScore)}</h2>
                <small>Overall assessment portfolio readiness</small>
              </div>
              <strong>
                {assuranceScore === null ? "—" : `${assuranceScore}%`}
              </strong>
            </div>
            <div className="pdPlanCard">
              <span>CURRENT PLAN</span>
              <h2>{getPlanLabel(subscription?.plan)}</h2>
              <p>
          ...
          <section className="pdMetrics">
            <Metric
              label="Audit readiness"
              value={assuranceScore === null ? "—" : `${assuranceScore}%`}
              detail={`${assessments.length} assessments`}
              href="/portal/history"
            />
            <Metric
              label="Open findings"
          ...
          <section className="pdGrid">
            <div className="pdPanel">
              <div className="pdPanelHead">
                <h2>Recent internal audits</h2>
                <Link href="/portal/internal-audits">View all →</Link>
              </div>
              {audits.length ? (
                audits.slice(0, 5).map((audit) => {
                  const standard =
                    audit.internal_audit_selected_standards?.[0]
                      ?.internal_audit_standard_catalogue?.display_name ??
                    "Internal audit";
                  return (
                    <div className="pdAudit" key={audit.id}>
                      <div>
                        <strong>{audit.title || audit.audit_reference}</strong>
                        <small>{audit.audit_reference}</small>
                      </div>
                      <span>{standard}</span>
                      <span className="pdPill">{label(audit.status)}</span>
                      <Link
                        href={`/portal/internal-audits/${audit.id}`}
                        className="pdOpen"
                      >
                        Open →
                      </Link>
                ...
                <p>No internal audits have been created yet.</p>
              )}
            </div>
            <div className="pdPanel">
              <div className="pdPanelHead">
                <h2>Assurance progress</h2>
              </div>
              <strong style={{ fontSize: 38, color: "#07945d" }}>
                {completionRate}%
              </strong>
              <p style={{ color: "#687d96" }}>
                Corrective actions independently verified effective
              </p>
              <div className="pdProgress">
                <i style={{ width: `${completionRate}%` }} />
              </div>
              <div className="pdProgressText">
                <span>{verifiedActions.length} verified</span>
                <span>{controlledActions.length} total</span>
              </div>
              <div className="pdQuick">
                <Link href="/portal/health-safety">
                  Health &amp; Safety Hub →
                </Link>
                <Link href="/portal/health-safety/training">My Training →</Link>
                <Link href="/portal/internal-audits">Internal Audit →</Link>
                <Link href="/portal/rca">CAPA-8D →</Link>
                <Link href="/portal/documents">Controlled Documents →</Link>
              </div>
            </div>
          </section>
