...
import RcaTrainingPlayer from "../../../../../components/RcaTrainingPlayer";
import { createClient } from "../../../../../lib/supabase/server";

export const metadata = { title: "RCA and Corrective Action Practitioner | RPG Excellence" };
...
async function loadCourse(supabase, userId, enrolmentId) {
  ...
  const [courseResult, modulesResult, progressResult] = await Promise.all([
    supabase.from("hs_training_courses").select("id,course_code,title,description,duration_minutes,pass_mark,validity_months,version,active,academy_code").eq("id", enrolment.course_id).eq("course_code", "RCA-8D-001").eq("academy_code", "rca_8d").eq("active", true).maybeSingle(),
...
async function completeModuleAction(formData) {
  ...
  if (!exerciseEvidence.correct || !exerciseEvidence.guidance_reviewed || String(exerciseEvidence.rationale || "").trim().length < 25) {
    throw new Error("Review the guidance and provide a supported RCA rationale before continuing.");
  }
  ...
  const { data: course, error: courseError } = await supabase.from("hs_training_courses").select("id,course_code,academy_code").eq("id", enrolment.course_id).eq("course_code", "RCA-8D-001").eq("academy_code", "rca_8d").single();
  if (courseError || !course) throw new Error("RCA–8D course access could not be verified.");
  ...
  revalidatePath(`/portal/rca/training/${enrolment.id}`);
  revalidatePath("/portal/rca/training");
  if (allComplete) redirect(`/portal/rca/training/${enrolment.id}/assessment`);
}

export default async function RcaCoursePage({ params }) {
  const { id } = await params;
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/portal/login?next=/portal/rca/training/${id}`);
  ...
  `}</style><div className="iacShell"><header className="iacTop"><div><small>RCA–8D · INTERACTIVE PRACTITIONER</small><h1>{record.course.title}</h1><p>{record.course.description}</p></div><div className="iacActions"><Link className="iacButton" href="/portal/rca/training">← Training Academy</Link><Link className="iacButton" href="/portal/rca">RCA–8D Hub</Link></div></header><RcaTrainingPlayer enrolment={record.enrolment} course={record.course} modules={record.modules} progress={record.progress} completeModuleAction={completeModuleAction}/><aside className="iacReminder"><strong>Evidence retained: </strong>Each checked RCA decision, learner rationale and guidance acknowledgement is saved against the enrolment as controlled module evidence.</aside></div></main>;
}
