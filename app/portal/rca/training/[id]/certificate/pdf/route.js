...
export async function GET(request, { params }) {
  ...
  if (!user) {
    return Response.redirect(new URL(`/portal/login?next=/portal/rca/training/${id}/certificate/pdf`, request.url), 303);
  }
  ...
  if (courseError) {
    return new Response("Unable to validate the RCA-8D Practitioner course.", { status: 500 });
  }
  ...
  if (
    ...
    return new Response("This certificate is not an RCA-8D Practitioner record.", { status: 404 });
  }
  ...
  pdf.setSubject("RCA and Corrective Action Practitioner certificate of completion");
  pdf.setKeywords(["RPG Excellence", "RCA", "8D", "corrective action", "certificate"]);
  ...
  drawCentered(page, "RCA-8D Practitioner Learning", serif, 35, height - 180, navy);
  ...
  drawCentered(page, "and achieving the required standard in the RCA-8D Practitioner assessment.", sans, 11, height - 366, grey);
  ...
  drawCentered(page, "Course completion supports RCA knowledge; competence also requires experience, observed practice, authority and evaluation.", sans, 7, 38, grey);
