...
export const metadata = {
  title: "RCA and Corrective Action Practitioner Assessment | RPG Excellence",
};
...
async function submitAssessment(formData) {
  ...
  if (ownedEnrolment.status === "passed") {
    redirect(`/portal/rca/training/${enrolmentId}/assessment?result=passed`);
  }
  ...
  if (courseResult.data.course_code !== "RCA-8D-001" || courseResult.data.academy_code !== "rca_8d") {
    throw new Error("This enrolment is not an RCA–8D Practitioner course.");
  }
  ...
  if (passed) {
    const certificateNumber = `RPG-RCA-${now.slice(0, 4)}-${crypto.randomUUID().replaceAll("-", "").slice(0, 10).toUpperCase()}`;
  ...
  revalidatePath(`/portal/rca/training/${enrolmentId}`);
  revalidatePath(`/portal/rca/training/${enrolmentId}/assessment`);
  revalidatePath("/portal/rca/training");
  redirect(`/portal/rca/training/${enrolmentId}/assessment?result=${passed ? "passed" : "failed"}&score=${score}`);
}
...
export default async function TrainingAssessmentPage({ params, searchParams }) {
  ...
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/portal/login?next=/portal/rca/training/${id}/assessment`);
  ...
  if (!["assessment_due", "failed", "passed"].includes(enrolment.status)) {
    redirect(`/portal/rca/training/${id}`);
  }
  ...
  if (course.course_code !== "RCA-8D-001" || course.academy_code !== "rca_8d") notFound();
  const questions = questionsResult.data || [];
    ...
    <main className="taPage">
      ...
      <div className="taWrap">
        <header className="taTop">
          <div>
            <small>RPG RCA–8D ACADEMY · FINAL ASSESSMENT</small>
            <h1>{course.title}</h1>
            <p>Answer every question. A score of at least {course.pass_mark}% is required to pass.</p>
          </div>
          <Link className="taLink" href={`/portal/rca/training/${id}`}>← Course content</Link>
        </header>
          ...
          <section className={`taResult ${passed ? "" : "fail"}`}>
            ...
            {certificate && (
              <div className="taCertificate">
                ...
                <Link href={`/portal/rca/training/${id}/certificate`}>
