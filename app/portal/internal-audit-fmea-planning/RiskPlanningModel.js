...
function ScoreSelect({ value, onChange, label }) {
  return <label className="rpmField"><span>{label}</span><select value={value} onChange={(event) => onChange(Number(event.target.value))}>{SCALE.map((score) => <option value={score} key={score}>{score}</option>)}</select></label>;
}

export default function RiskPlanningModel() {
  ...
  return <div className="rpmModel">
    <section className="rpmPanel">
      ...
      <div className="rpmBody rpmGrid2">
        <label className="rpmField"><span>Process / activity *</span><input placeholder="e.g. Supplier approval and monitoring" /></label>
        <label className="rpmField"><span>Site / central function</span><input placeholder="e.g. Port Talbot / Group IT" /></label>
        <label className="rpmField"><span>Credible failure mode *</span><textarea placeholder="Describe how the process or control could fail—not merely the observed symptom." /></label>
        <label className="rpmField"><span>Credible effects *</span><textarea placeholder="Describe potential effects on customers, compliance, people, environment, security, continuity or certification integrity." /></label>
        <label className="rpmField rpmWide"><span>Existing preventive and detective controls *</span><textarea placeholder="Identify current controls, their owners, monitoring frequency and the evidence that they operate effectively." /></label>
        <label className="rpmField rpmWide"><span>Applicable standards</span><div className="rpmChecks">{STANDARDS.map((standard) => <label key={standard}><input type="checkbox" /> {standard}</label>)}</div></label>
        <label className="rpmField rpmWide"><span>Evidence considered *</span><textarea placeholder="Reference KPIs, previous audits, findings, incidents, complaints, legal obligations, changes, risk registers and management concerns." /></label>
      </div>
    </section>
    ...
    <section className="rpmPanel">
      ...
      <div className="rpmBody rpmConsequenceGrid">{CONSEQUENCES.map(([key, title, help]) => <article key={key} className={consequences[key] === consequence ? "rpmFactor selected" : "rpmFactor"}><ScoreSelect label={title} value={consequences[key]} onChange={(value) => { setConsequences((current) => ({...current, [key]: value})); setConfirmed(false); }} /><small>{help}</small></article>)}</div>
    ...
    <section className="rpmPanel">
      ...
      <div className="rpmBody rpmGrid2">
        <div className="rpmFactor"><ScoreSelect label="Likelihood" value={likelihood} onChange={(value) => { setLikelihood(value); setConfirmed(false); }} /><p><b>1:</b> exceptional · <b>2:</b> unlikely · <b>3:</b> possible · <b>4:</b> likely/recurrent · <b>5:</b> current or expected</p></div>
        <div className="rpmFactor"><ScoreSelect label="Control detectability" value={detectability} onChange={(value) => { setDetectability(value); setConfirmed(false); }} /><p><b>1:</b> real-time independent detection · <b>2:</b> strong routine monitoring · <b>3:</b> periodic monitoring · <b>4:</b> weak/reactive · <b>5:</b> no reliable detection</p></div>
      </div>
    ...
    <section className="rpmPanel">
      ...
      <div className="rpmBody rpmGrid2">
        <label className="rpmField"><span>Lead auditor</span><input placeholder="Name of administering lead auditor" /></label>
        <label className="rpmField"><span>Decision</span><select defaultValue="accept"><option value="accept">Accept calculated recommendation</option><option value="increase">Increase audit priority/frequency</option><option value="decrease">Decrease audit priority/frequency</option></select></label>
        <label className="rpmField rpmWide"><span>Decision and override rationale *</span><textarea value={override} onChange={(event) => { setOverride(event.target.value); setConfirmed(false); }} placeholder="Explain the evidence, assumptions, uncertainty and why the selected audit timing provides appropriate assurance." /></label>
        <label className="rpmConfirm rpmWide"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /><span>I confirm that the scoring is evidence-based, overlapping impacts have not been double-counted, and the audit frequency remains a lead-auditor decision.</span></label>
      </div>
      <div className="rpmActions"><button type="button" className="rpmGhost" onClick={() => window.print()}>Print / Save Assessment</button><span className={confirmed && override.trim() ? "rpmReady" : "rpmPending"}>{confirmed && override.trim() ? "Decision ready to record" : "Rationale and confirmation required"}</span></div>
    </section>
  </div>;
}
