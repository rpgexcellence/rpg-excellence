...
export default async function ISMSRiskPage(){
 ...
 const [riskResult,assetResult,controlResult,assessmentResult,linkResult,treatmentResult]=await Promise.all([
 ...
 for(const result of [riskResult,assetResult,controlResult,assessmentResult,linkResult,treatmentResult])if(result.error)throw new Error(result.error.message);
 return <main className="irmPage"><style>{`.irmPage{min-height:100vh;padding:27px clamp(14px,3vw,46px) 80px;background:#edf3f8;font-family:Arial,sans-serif;color:#071d3a}.irmTop{max-width:1500px;margin:0 auto 20px;display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.irmTop small{color:#07859a;font-weight:950;letter-spacing:.12em}.irmTop h1{margin:7px 0;font-size:40px}.irmTop p{margin:0;color:#60768c}.irmTop div:last-child{display:flex;gap:8px;flex-wrap:wrap}.irmTop a{padding:11px 14px;border:1px solid #c9d7e4;border-radius:8px;background:#fff;color:#153b60;text-decoration:none;font-weight:850}@media(max-width:650px){.irmPage{padding:16px 10px 55px;overflow-x:hidden}.irmTop{display:block}.irmTop h1{font-size:30px}.irmTop div:last-child{margin-top:14px}.irmTop a{flex:1;text-align:center}}`}</style><header className="irmTop"><div><small>ISMS HUB · ISO/IEC 27001 RISK MANAGEMENT</small><h1>Information Security Risk Management</h1><p>{org.name} · risks, treatment, SoA controls and accountable acceptance.</p></div><div><Link href="/portal/information-security">← ISMS Hub</Link><Link href="/portal/soa">SoA Register</Link></div></header><ISMSRiskWorkspace action={saveISMSRisk} methodology={methodology} risks={riskResult.data||[]} assets={assetResult.data||[]} controls={controlResult.data||[]} assessments={assessmentResult.data||[]} links={linkResult.data||[]} treatments={treatmentResult.data||[]}/></main>
}
