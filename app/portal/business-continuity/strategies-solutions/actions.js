...
export async function saveStrategiesSolutions(_state, fd) {
  ...
  const approverPersonId = text("approver_person_id");
  const selectedPersonIds = [...new Set([...resources.map((x)=>clean(x.ownerPersonId)), ...actions.map((x)=>clean(x.ownerPersonId)), approverPersonId].filter(Boolean))];
  let companyPeople = [];
  if(selectedPersonIds.length){const{data,error}=await s.from("organization_people").select("id,first_name,last_name,account_status").eq("organization_id",org.id).in("id",selectedPersonIds).not("account_status","in",'(suspended,closed)');if(error)return{error:error.message};companyPeople=data||[];if(companyPeople.length!==selectedPersonIds.length)return{error:"One or more selected owners are no longer available in Company People."}}
  const personById = new Map(companyPeople.map((person)=>[person.id,person]));
  const personName = (id) => { const person=personById.get(id); return person ? `${person.first_name} ${person.last_name}` : ""; };
  for(const resource of resources)if(resource.ownerPersonId)resource.owner=personName(resource.ownerPersonId);
  for(const action of actions)if(action.ownerPersonId)action.owner=personName(action.ownerPersonId);
  ...
  if(intent==="approve"){if(!approverPersonId||!reviewer)return{error:"Select an authorised BCP Leader as the competent approver."};const{data:leader,error:leaderError}=await s.from("organization_person_authorizations").select("id,expires_at").eq("organization_id",org.id).eq("person_id",approverPersonId).eq("function_key","bcp_leader").eq("status","authorised").maybeSingle();if(leaderError)return{error:leaderError.message};if(!leader||(leader.expires_at&&leader.expires_at<new Date().toISOString().slice(0,10)))return{error:"The selected approver does not have a current BCP Leader authorisation."}}
