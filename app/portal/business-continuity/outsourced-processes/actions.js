...
export async function saveOutsourcedProcesses(_state, fd) {
  ...
  const ids={profile:t("site_profile_id"),context:t("context_assessment_id"),role:t("role_assessment_id"),hazard:t("hazard_assessment_id"),bia:t("bia_assessment_id")};
  const load=(table,key)=>ids[key]?s.from(table).select("*").eq("id",ids[key]).eq("organization_id",org.id).neq("status","archived").maybeSingle():Promise.resolve({data:null});
  const[{data:profile},{data:context},{data:role},{data:hazard},{data:bia}]=await Promise.all([load("bcp_site_profiles","profile"),load("bcp_context_assessments","context"),load("bcp_role_assessments","role"),load("bcp_hazard_assessments","hazard"),load("bcp_bia_assessments","bia")]);
  if(!profile||!bia)return{error:"Select valid Module 1 and Module 6 controlled records."};
  ...
  if(intent==="approve"){for(const[record,label]of[[profile,"Module 1 Site Profile"],[context,"Module 3 Context"],[role,"Module 4 Roles"],[hazard,"Module 5 Hazards"],[bia,"Module 6 BIA"]])if(record&&record.status!=="approved")return{error:`The linked ${label} must be approved first.`}}
