...
export async function saveOutsourcedProcesses(_state, fd) {
  ...
  if(linkedSupplierIds.length){
    ...
    const{data:approved,error:approvedError}=await s.from("suppliers").select("id,legal_name").eq("organization_id",org.id).eq("owner_id",user.id).eq("approval_status","approved").in("id",linkedSupplierIds);

    if(approvedError)return{error:approvedError.message};

    if((approved||[]).length!==linkedSupplierIds.length)return{error:"A linked supplier is no longer approved. Select an approved supplier or switch that record to manual entry."};

    const byId=new Map(approved.map((supplier)=>[supplier.id,supplier]));

    for(const control of controls)if(control.supplierId)control.supplierName=byId.get(control.supplierId).legal_name;
