...
const PROFILE_EXTENTS = new Set(["isolated", "process_wide", "site_wide", "organisation_wide", "external_interface"]);
const CONTROL_LAYERS = new Set(["prevention", "detection", "response", "recovery"]);
const RECURRENCE_RELATIONSHIPS = new Set(["first_occurrence", "similar_previous", "confirmed_recurrence", "unknown"]);
const FAILURE_MECHANISMS = new Set([
  "absent", "inadequate_design", "not_implemented", "inconsistent", "bypassed", "not_understood",
  "unavailable", "obsolete_after_change", "degraded", "monitoring_failed", "escalation_failed", "prior_action_ineffective",
]);
...
function parseCauseProfile(formData) {
  ...
  const applicableStandards = [...new Set(formData.getAll("applicable_standards").map(clean).filter((item) => PROFILE_STANDARDS.has(item)))];
  if (!selected || selected.category !== profileCategory || !FAILURE_MECHANISMS.has(failureMechanism) ||
      !affectedProcess || !PROFILE_EXTENTS.has(profileExtent) || !CONTROL_LAYERS.has(controlLayer) ||
      !RECURRENCE_RELATIONSHIPS.has(recurrenceRelationship) || !accountableSystemOwner || !profileRationale ||
      applicableStandards.length === 0) {
    throw new Error("Choose a valid legacy code and complete every required RCA profile field.");
  }
  return {
    profile_category: selected.category, profile_code: selected.code, profile_title: selected.title,
    failure_mechanism: failureMechanism, affected_process: affectedProcess, profile_extent: profileExtent,
    control_layer: controlLayer, recurrence_relationship: recurrenceRelationship,
    accountable_system_owner: accountableSystemOwner, applicable_standards: applicableStandards,
...
async function getOwnedCase(supabase, userId, caseId) {
  ...
  const { data: assignment, error: assignmentError } = await supabase
    .from("internal_audit_action_access").select("rca_case_id, owner_id")
    .eq("secure_token_hash", tokenHash(token || ""))
    .gt("secure_token_expires_at", new Date().toISOString()).maybeSingle();
  if (assignmentError || !assignment || assignment.rca_case_id !== caseId || assignment.owner_id !== userId) {
    throw new Error("This secure link is not authorised for the requested 8D case.");
  }
...
async function assertDisciplineUnlocked(
  ...
  if (blocker) redirect(await secureCasePath(caseId, `?d=${blocker.discipline}&locked=1`));
}

const tokenHash = (token) => createHash("sha256").update(token).digest("hex");

async function secureToken() {
  return (await cookies()).get("audit_action_token")?.value || null;
}

async function secureCasePath(caseId, suffix = "") {
  const token = await secureToken();
  if (!token) throw new Error("The secure action session has expired. Reopen the assigned link.");
  return `/audit-actions/${token}${suffix}`;
}
...
async function context() {
  ...
  const { data: access, error } = await supabase.from("internal_audit_action_access")
    .select("*").eq("secure_token_hash", tokenHash(token))
    .gt("secure_token_expires_at", new Date().toISOString()).maybeSingle();
  if (error || !access?.rca_case_id) throw new Error(error?.message || "The secure action link is invalid or expired.");
  if (["submitted", "accepted", "closed"].includes(access.status)) {
    throw new Error("The final D0-D8 response has been submitted and is read only pending auditor review.");
  }
  return { supabase, user: { id: access.owner_id }, access, token };
}
...
export async function saveCaseOverview(formData) {
  ...
  redirect(await secureCasePath(caseId));
}

export async function saveDiscipline(formData) {
  const { supabase, user, access } = await context();
  const caseId = clean(formData.get("case_id"));
  ...
  if (!narrative) {
    redirect(await secureCasePath(caseId, `?d=${discipline}&error=narrative_required`));
  }
  ...
  if (intent === "approve" && discipline === 4) {
    ...
    if (missingTypes.length > 0) {
      ...
      redirect(await secureCasePath(caseId, `?d=4${modelQuery}&error=missing_validated_causes&missing=${encodeURIComponent(missingTypes.join(","))}`));
    }
    if ((validatedCauses ?? []).some((cause) => !cause.profile_code || !cause.profile_rationale)) {
      redirect(await secureCasePath(caseId, "?d=4&error=missing_cause_profiles"));
    }
  }
  ...
  if (intent === "approve" && discipline === 5) {
    ...
    if (selectedActions.length === 0 || uncoveredTypes.length > 0 || incompleteSelectedAction) {
      redirect(await secureCasePath(caseId, `?d=5&error=d5_selection_incomplete&missing=${encodeURIComponent([...new Set(uncoveredTypes)].join(","))}`));
    }
  }

  if (intent === "approve" && discipline === 6) {
    const { data: verifiedActions, error: verifiedActionsError } = await supabase
      .from("rca_actions")
      .select("id, effectiveness_result")
      .eq("case_id", caseId)
      .eq("owner_id", user.id)
      .eq("discipline", 5)
      .eq("selection_status", "selected");
  ...
  const status = approved && discipline === 8
    ? "ready_for_review"
    : intent === "review"
      ? "ready_for_review"
      : "in_progress";
  ...
  if (approved) {
    ...
    if (discipline === 8) {
      const { data: completedStages, error: completedStagesError } = await supabase
        .from("rca_8d_disciplines").select("discipline, narrative, status")
        .eq("case_id", caseId).eq("owner_id", user.id).order("discipline");
      if (completedStagesError) throw new Error(completedStagesError.message);
      const byNumber = new Map((completedStages ?? []).map((item) => [item.discipline, item]));
      const missing = Array.from({ length: 9 }, (_, number) => number)
        .filter((number) => byNumber.get(number)?.status !== "approved" || !byNumber.get(number)?.narrative?.trim());
      if (missing.length) {
        redirect(await secureCasePath(caseId, `?d=8&error=incomplete_8d&missing=${missing.join(",")}`));
      }
      const now = new Date().toISOString();
      const { error: accessError } = await supabase.from("internal_audit_action_access").update({
        status: "submitted",
        correction_and_containment: [byNumber.get(0)?.narrative, byNumber.get(3)?.narrative].filter(Boolean).join("\n\n"),
        root_cause_response: byNumber.get(4)?.narrative,
        corrective_action_plan: [byNumber.get(5)?.narrative, byNumber.get(6)?.narrative, byNumber.get(7)?.narrative].filter(Boolean).join("\n\n"),
        effectiveness_measure: byNumber.get(8)?.narrative,
      ...
      const { error: draftError } = await supabase.from("internal_audit_action_access").update({
        status: "response_draft", acknowledged_at: access.acknowledged_at || new Date().toISOString(), updated_at: new Date().toISOString(),
      }).eq("id", access.id);
  ...
  await supabase.from("rca_case_events").insert({
    ...
    owner_id: user.id,
    event_type: approved
      ? discipline === 8 ? "action_owner_8d_submitted" : "action_owner_discipline_completed"
      : "discipline_updated",
    discipline,
    summary: approved
      ? discipline === 8 ? "Complete D0-D8 response submitted for auditor review" : `D${discipline} completed by action owner`
  ...
  const modelQuery = discipline === 4 && modelId
    ...
    : "";
  redirect(await secureCasePath(caseId, `?d=${discipline}${modelQuery}`));
}
...
export async function addTeamMember(formData) {
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=1`));
}
...
export async function addCauseHypothesis(formData) {
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=4`));
}
...
export async function reviewCauseHypothesis(formData) {
  ...
  if (decision === "validate" && (!validationMethod || !validationResult)) {
    const modelQuery = modelId ? `&model=${encodeURIComponent(modelId)}` : "";
    redirect(await secureCasePath(caseId, `?d=4${modelQuery}&error=cause_validation_required&cause=${encodeURIComponent(causeId)}`));
  }
  ...
  if (decision === "validate") {
    const { data: profiledCause, error: profileError } = await supabase
      ...
      .eq("id", causeId).eq("case_id", caseId).eq("owner_id", user.id).maybeSingle();
    if (profileError) throw new Error(profileError.message);
    if (!profiledCause?.profile_code || !profiledCause?.profile_rationale) {
      redirect(await secureCasePath(caseId, `?d=4&error=cause_profile_required&cause=${encodeURIComponent(causeId)}`));
    }
  }
  ...
  revalidatePath(`/portal/rca/${caseId}`);
  const modelQuery = modelId ? `&model=${encodeURIComponent(modelId)}` : "";
  redirect(await secureCasePath(caseId, `?d=4${modelQuery}`));
}
...
export async function saveCauseProfile(formData) {
  ...
  const { data: cause, error: causeError } = await supabase
    .from("rca_causes").select("id, cause_type, status")
    .eq("id", causeId).eq("case_id", caseId).eq("owner_id", user.id).maybeSingle();
  if (causeError) throw new Error(causeError.message);
  if (!cause) throw new Error("Cause hypothesis not found.");
  const wasValidated = cause.status === "validated";
  const update = { ...profile };
  if (wasValidated) Object.assign(update, {
    status: "under_test", validation_method: null, validation_result: null,
    validated_by: null, validated_at: null,
  });
  const { error } = await supabase.from("rca_causes").update(update)
    .eq("id", causeId).eq("case_id", caseId).eq("owner_id", user.id);
  if (error) throw new Error(error.message);
  await supabase.from("rca_case_events").insert({
    case_id: caseId, owner_id: user.id, event_type: "cause_profile_saved", discipline: 4,
...
export async function addCorrectiveAction(formData) {
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=${discipline}`));
}
...
export async function submitD6ActionForVerification(formData) {
  ...
  const { error: accessError } = await supabase.from("internal_audit_action_access").update({
    d6_verification_requested_at: now,
    d6_verification_completed_at: null,
    updated_at: now,
  }).eq("id", access.id).eq("rca_case_id", caseId);
  if (accessError) throw new Error(accessError.message);
  await supabase.from("rca_case_events").insert({
    case_id: caseId, owner_id: user.id, event_type: "d6_action_submitted",
    discipline: 6, summary: `${action.title} submitted for auditor effectiveness verification`,
    event_data: { action_id: action.id, action_access_id: access.id },
  });
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, "?d=6&saved=verification_requested"));
}
...
export async function decideCorrectiveActionCandidate(formData) {
  ...
  if (decision === "select" && !rationale) {
    redirect(await secureCasePath(caseId, `?d=5&error=selection_rationale_required`));
  }
  ...
  if (
    ...
    redirect(await secureCasePath(caseId, `?d=5&error=candidate_fields_required`));
  }
  ...
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=5`));
}
...
export async function recordNoContainmentRequired(formData) {
  ...
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=3`));
}
...
export async function createAnalysisModel(formData) {
  ...
  if (existing) {
    redirect(await secureCasePath(caseId, `?d=4&model=${existing.id}`));
  }
  ...
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=4&model=${model.id}`));
}
...
export async function addAnalysisNode(formData) {
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/portal/rca/${caseId}`);
  redirect(await secureCasePath(caseId, `?d=4&model=${modelId}`));
}
...
export async function reviewAnalysisNode(formData) {
  ...
  if (decision === "validate" && (!validationMethod || !validationResult)) {
    redirect(await secureCasePath(caseId, `?d=4&model=${encodeURIComponent(modelId)}&error=node_validation_required&node=${encodeURIComponent(nodeId)}`));
  }
...
export async function addCostEntry(formData) {
  ...
  redirect(await secureCasePath(caseId, `?d=${discipline}#copq`));
}
