...
export default async function SecureRcaCasePage({
  params,
  searchParams,
}) {
  ...
  const { data: access, error: accessError } = await supabase
    .from("internal_audit_action_access").select("*")
    .eq("secure_token_hash", hash)
    .gt("secure_token_expires_at", new Date().toISOString()).maybeSingle();
  if (accessError) throw new Error(`Secure action lookup failed: ${accessError.message}`);
  if (!access?.rca_case_id) return <main style={{minHeight:"100vh",display:"grid",placeItems:"center",padding:24,background:"#f3f6fa",color:"#061a35"}}><section style={{maxWidth:650,padding:32,border:"1px solid #d5e1ed",borderRadius:18,background:"white"}}><h1>Secure action link unavailable</h1><p>This restricted link is invalid, expired or is not connected to a controlled RCA case. Ask the lead auditor to issue a new link.</p></section></main>;
  const id = access.rca_case_id;
  const user = { id: access.owner_id };
  ...
  if (!caseResult.data) throw new Error("The linked controlled RCA case is unavailable.");
  for (const result of [
    disciplinesResult,
    teamResult,
    causesResult,
  ...
  if (requestedDiscipline > highestUnlocked) {
    redirect(`/audit-actions/${token}?d=${highestUnlocked}&locked=1`);
  }
  ...
  const frozen = ["submitted", "accepted", "closed"].includes(access.status);
      ...
      <div style={{ maxWidth: "1500px", margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <Link href="#discipline-workspace" style={primaryLinkButton}>
              Secure 8D Workspace
            </Link>
            <Link href="#case-overview" style={linkButton}>
              Case Overview
            </Link>
          </div>
        </div>
        ...
        >
          {[
            ...
            ["Owner-completed disciplines", `${approvedCount}/9`],
        ...
        >
          ...
          >
              {disciplines.map((item) => {
                ...
                if (locked) {
                  return (
                    <div
                      key={item.id}
                      aria-disabled="true"
                      title={`Complete D${item.discipline - 1} to unlock D${item.discipline}.`}
                ...
                return (
                  <Link
                    key={item.id}
                    href={`/audit-actions/${token}?d=${item.discipline}`}
          ...
          <div id="discipline-workspace" style={{ minWidth: 0 }}>
            {frozen && <div style={{...validationNoticeStyle,background:"#e8f8ef",borderColor:"#9bd8b7",color:"#07613a"}}><strong>Final D0–D8 response submitted.</strong><div style={{marginTop:6}}>The controlled investigation is frozen while the auditor reviews the validated root cause, corrective-action plan and objective evidence.</div></div>}
            {discipline && (
              <section style={cardStyle}>
                ...
                {pageError === "d5_selection_incomplete" && (
                  <div style={validationNoticeStyle}>
                    <strong>D5 cannot be completed yet.</strong>
                    <div style={{ marginTop: "6px" }}>
                ...
                <div style={reviewResultStyle}>
                  ...
                  <div style={{ color: "#607089", marginTop: "14px" }}>
                    Rules-based advisory review only · The action owner remains accountable for the investigation and evidence. Auditor review occurs after final D8 submission.
                  </div>
                </div>
            ...
            {selected === 6 && (
              <D6EffectivenessWorkbench
                ...
                evidenceRecords={evidenceRecords.filter((record) => record.discipline === 6)}
              />
            )}
          ...
          <article id={`d6-action-${action.id}`} key={action.id} style={{ ...candidateCardStyle(["effective", "effective_verified"].includes(action.effectiveness_result) ? "selected" : "candidate"), scrollMarginTop: "24px" }}>
            ...
            <div style={{ marginTop: "14px", padding: "14px", border: "1px solid #cbd8ea", borderRadius: "12px", background: "#f8fbff" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
                ...
                <span style={evidenceCountStyle}>{actionEvidence.length} file{actionEvidence.length === 1 ? "" : "s"}</span>
              </div>
              ...
              <form action={addObjectiveEvidence} encType="multipart/form-data" style={{ marginTop: "14px" }}>
                <input type="hidden" name="case_id" value={caseId} />
                <input type="hidden" name="discipline" value="6" />
                <input type="hidden" name="action_id" value={action.id} />
                <label style={dropZoneStyle}><strong>Attach evidence to this action</strong><span style={{ color: "#607089", fontSize: "13px" }}>PDF, Word, Excel, CSV, text or images · maximum 10 files · 10 MB each</span><input type="file" name="evidence_files" multiple required accept=".pdf,.docx,.xlsx,.csv,.txt,.png,.jpg,.jpeg,.webp" style={{ marginTop: "10px" }} /></label>
                <div style={formGrid}><input name="reference" placeholder="Evidence reference (optional)" style={fieldStyle} /><input type="date" name="evidence_date" style={fieldStyle} /><select name="strength" defaultValue="medium" style={fieldStyle}><option value="low">Low evidence strength</option><option value="medium">Medium evidence strength</option><option value="high">High evidence strength</option></select></div>
                <textarea name="description" required rows={3} placeholder="Describe what this evidence demonstrates for this corrective action and its effectiveness criteria." style={{ ...fieldStyle, marginTop: "12px", resize: "vertical" }} />
                <button type="submit" style={{ ...primaryButton, marginTop: "12px" }}>Add Evidence to This Action</button>
              </form>
            </div>
...
function buildEvidenceChallenge({
  ...
  return {
    ...
    assessment:
      issueCount === 0
        ? "The recorded structure meets the configured checks. Review content quality and objective evidence before completing this discipline."
        : "Resolve or consciously address the points below before completing this discipline.",
    challenges,
    missingEvidence,
