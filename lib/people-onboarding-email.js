...
export async function sendWelcomeEmail({ person, organisation, functions, permissions, manager }) {
  ...
  await sendEmail({
    ...
    html: baseEmail(`Welcome to RPG Excellence`, `<p>Hello ${escapeHtml(person.first_name)},</p><p>Your account has been verified and connected to <b>${escapeHtml(organisation.name)}</b>.</p><div style="margin:22px 0;padding:16px;background:#f3f7fb;border-left:4px solid #10a37f"><b>Position:</b> ${escapeHtml(person.position || "Not specified")}<br/><b>Reports to:</b> ${escapeHtml(manager ? `${manager.first_name} ${manager.last_name}` : "Not assigned")}<br/><b>Authorised functions:</b> ${escapeHtml(roles.join(", ") || "None assigned")}<br/><b>Platform access:</b> ${escapeHtml(modules.join(", ") || "No module access assigned")}</div><a href="${site}/portal/my-access" style="display:inline-block;padding:13px 18px;border-radius:9px;background:#315fe6;color:#fff;text-decoration:none;font-weight:800">Open my authorised access</a><p style="margin-top:22px;color:#5f7185;font-size:13px">Your password is never included in an RPG email. Privileged users should enable multi-factor authentication.</p>`),
    text: `Hello ${person.first_name},\n\nYour RPG Excellence account is active and connected to ${organisation.name}.\n\nAuthorised functions: ${roles.join(", ") || "None assigned"}\nPlatform access: ${modules.join(", ") || "No module access assigned"}\n\nOpen ${site}/portal/my-access`,
  });
}
