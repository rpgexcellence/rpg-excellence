...
function ProcessRepeater({ items, setItems, support=false, services=[], relatedProcesses=[] }) {
  ...
  return <div className="spxRepeater">{!support&&processNames.length>0&&<div className="spxChain">{processNames.map((name,i)=><span key={name+i}><b>{name}</b>{i<processNames.length-1&&<i>→</i>}</span>)}</div>}{items.map((x,i)=>{
    ...
    return <article key={i}><header><div><b>{String(i+1).padStart(2,"0")}</b><strong>{x.name||((support?"Support":"Value-chain")+" process "+(i+1))}</strong></div><button type="button" onClick={()=>setItems(items.filter((_,n)=>n!==i))}>Remove</button></header><div className="spxGrid">
      ...
      <label>Process owner<select value={x.owner||""} onChange={e=>update(i,"owner",e.target.value)}><option value="">Select accountable role</option>{ownerRoles.map(r=><option key={r}>{r}</option>)}</select></label>
      ...
      <label>Upstream process or input<select value={x.upstream||""} onChange={e=>update(i,"upstream",e.target.value)}><option value="">Select preceding process or input</option><option>Customer enquiry</option>{processNames.filter(n=>n!==x.name).map(n=><option key={n}>{n}</option>)}<option>External supplier or trigger</option><option>Other</option></select></label>
      ...
      <label>Downstream process or recipient<select value={x.downstream||""} onChange={e=>update(i,"downstream",e.target.value)}><option value="">Select receiving process or party</option>{processNames.filter(n=>n!==x.name).map(n=><option key={n}>{n}</option>)}<option>Customer</option><option>Regulator or accreditation body</option><option>Operational planning</option><option>Other</option></select></label>
...
components/BCPSiteProfileForm.js
