...
const peopleGroups = ["Employees", "Contractors", "Visitors", "Members of the public", "Young persons", "New or expectant mothers", "Disabled persons", "Lone workers", "Others"];
const likelihoodOptions = [[1, "Very unlikely — exceptional circumstances"], [2, "Unlikely — could occur but is not expected"], [3, "Possible — may occur occasionally"], [4, "Likely — expected in normal circumstances"], [5, "Very likely — expected frequently or repeatedly"]];
const severityOptions = [[1, "No injury or negligible harm"], [2, "First-aid injury"], [3, "Short-term absence injury"], [4, "Major injury or long-term harm"], [5, "Fatality or multiple serious injuries"]];
    ...
    <section className="hreAdd">
      ...
      {open && <form className="hreForm" action={addHazardAction}>
        ...
        <section className="hreStep"><div className="hreStepHead"><span>STEP 2 · IDENTIFY</span><h3>Hazard, exposure and credible harm</h3><p>Describe the source of harm and the people who could be affected.</p></div><div className="hreGrid">
          ...
          <fieldset className="hreField full" style={{border:0,padding:0,margin:0}}><legend className="hreLegend">Who may be harmed? Select all that apply *</legend><div className="hreChecks">{peopleGroups.map((group) => <label className="hreCheck" key={group}><input type="checkbox" name="people_exposed" value={group}/>{group}</label>)}</div></fieldset>
          <div className="hreField"><label htmlFor="people_exposed_other">Other people who may be harmed</label><input id="people_exposed_other" name="people_exposed_other" placeholder="Complete when Others is selected"/></div>
        ...
        <section className="hreStep"><div className="hreStepHead"><span>STEP 3 · EVALUATE</span><h3>Current controls and initial risk</h3><p>Score the risk with existing controls operating as they do today.</p></div><div className="hreGrid"><div className="hreField full"><label htmlFor="existing_controls">Existing controls *</label><textarea id="existing_controls" name="existing_controls" required placeholder="Record controls that are present, implemented and verifiable."/></div></div><div className="hreScoreGrid">
          <div className="hreField"><label htmlFor="current_likelihood">Likelihood *</label><select id="current_likelihood" name="current_likelihood" value={likelihood} onChange={(event) => setLikelihood(event.target.value)} required><option value="">Select likelihood</option>{likelihoodOptions.map(([number,text]) => <option value={number} key={number}>{number} — {text}</option>)}</select></div>
          <div className="hreField"><label htmlFor="current_severity">Severity *</label><select id="current_severity" name="current_severity" value={severity} onChange={(event) => setSeverity(event.target.value)} required><option value="">Select severity</option>{severityOptions.map(([number,text]) => <option value={number} key={number}>{number} — {text}</option>)}</select></div>
          <ScoreCard title="Initial risk" severity={severity} likelihood={likelihood}/>
        </div><div className="hreGuidance"><div className="hreGuide"><strong>Likelihood guide</strong>{likelihoodOptions.map(([number,text]) => <span key={number}><b>{number}</b> — {text}</span>)}</div><div className="hreGuide"><strong>Severity guide</strong>{severityOptions.map(([number,text]) => <span key={number}><b>{number}</b> — {text}</span>)}</div></div></section>
        <section className="hreStep"><div className="hreStepHead"><span>STEP 4 · CONTROL</span><h3>Additional controls and residual risk</h3><p>Work down the hierarchy and score the expected risk after controls are implemented.</p></div><fieldset style={{border:0,padding:0,margin:"0 0 15px"}}><legend className="hreLegend">Control hierarchy applied *</legend><div className="hreChecks">{hierarchy.map((item) => <label className="hreCheck" key={item}><input type="checkbox" name="control_hierarchy" value={item} onChange={(event) => event.target.checked ? selectedHierarchy.add(item) : selectedHierarchy.delete(item)}/>{item}</label>)}</div></fieldset><div className="hreGrid"><div className="hreField full"><label htmlFor="additional_controls">Additional controls required</label><textarea id="additional_controls" name="additional_controls" placeholder="Define specific measures needed to eliminate or reduce the risk."/></div></div><div className="hreScoreGrid">
          <div className="hreField"><label htmlFor="residual_likelihood">Residual likelihood *</label><select id="residual_likelihood" name="residual_likelihood" value={residualLikelihood} onChange={(event) => setResidualLikelihood(event.target.value)} required><option value="">Select likelihood</option>{likelihoodOptions.map(([number,text]) => <option value={number} key={number}>{number} — {text}</option>)}</select></div>
          <div className="hreField"><label htmlFor="residual_severity">Residual severity *</label><select id="residual_severity" name="residual_severity" value={residualSeverity} onChange={(event) => setResidualSeverity(event.target.value)} required><option value="">Select severity</option>{severityOptions.map(([number,text]) => <option value={number} key={number}>{number} — {text}</option>)}</select></div>
